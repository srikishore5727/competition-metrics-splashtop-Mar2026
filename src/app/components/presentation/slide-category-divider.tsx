...
export function SlideCategoryDivider({ onNavigateHome, onNavigate }: SlideCategoryDividerProps) {
  const categories = [
    { name: 'Remote Access',        color: '#EF4444', icon: Shield,  slideIndex: 8  },
    { name: 'Remote Desktop',       color: '#FF7AB6', icon: Network, slideIndex: 9  },
    { name: 'Remote Support',       color: '#7ED957', icon: Shield,  slideIndex: 10 },
    { name: 'Remote Connection',    color: '#FFB14A', icon: Cloud,   slideIndex: 11 },
    { name: 'Secure Remote Access', color: '#6C9AFF', icon: Lock,    slideIndex: 12 },
  ];

  return (
    <SlideContainer slideNumber={6} onNavigateHome={onNavigateHome} source="">
      <div className="flex-1 flex flex-col justify-center items-center px-12">
        ...
        <p className="text-xl text-gray-600 mb-14 text-center max-w-3xl">
          Deep-dive analysis across five key remote access & security market segments
        </p>

        {/* Category Grid — flex-wrap, 3 on row 1, 2 centred on row 2 */}
        <div className="flex flex-wrap justify-center gap-6 max-w-3xl w-full">
          {categories.map((category, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative bg-white border-2 border-gray-200 rounded-2xl p-6 hover:border-gray-300 hover:shadow-xl transition-all duration-300 cursor-pointer w-52"
